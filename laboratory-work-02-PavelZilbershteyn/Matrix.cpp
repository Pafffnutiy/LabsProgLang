...
{
    if (!(this->rows == right.rows && this->cols == right.cols)) throw std::runtime_error("Matrices have different sizes");
...
Matrix Matrix::operator*(const Matrix& right) const
{
    if (!(this->cols == right.rows)) throw std::runtime_error("Left cols != Right rows");
...
{
    if (!((row < this->rows) && (col < this->cols))) throw std::runtime_error("There is no such element");
    double val = 0.0;
...
double& Matrix::operator()(std::size_t row, std::size_t col)
{
    ...
    return coefs[coefs.size() - 1].value;
