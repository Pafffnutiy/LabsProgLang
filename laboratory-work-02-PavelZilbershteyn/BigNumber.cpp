...
{
	if (!(this->number.size() != 0)) throw std::runtime_error("Number is empty");
...
BigNumber  BigNumber::operator* (const BigNumber& right) const
{
	...
	{
		unsigned short digit = resultVec[i];
		if (digit >= 10)
		{
...
BigNumber  BigNumber::operator/ (const BigNumber& right) const
{
	if (this->absol() < right.absol()) return BigNumber(0);
...
{
	if (!(NewNum.size() != 0)) throw std::runtime_error("String is empty");
	if (NewNum[0] == '-')
	{
		this->sign = 1;
