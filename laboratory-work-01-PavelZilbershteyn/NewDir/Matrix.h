...
public:
   ...
   ~Matrix() = default;
	...
	Matrix operator*(int lambda) const;
	Matrix operator*(const Matrix& right) const;
	friend Matrix operator*(int lambda, const Matrix& right);
	...
	int  operator()(std::size_t row, std::size_t col) const; //если нужно как rvalue
	int& operator()(std::size_t row, std::size_t col); //если нужно как lvalue
};
