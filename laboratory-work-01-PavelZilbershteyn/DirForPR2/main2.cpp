...
int main()
{
	{ //TEST1. Проверка работоспособности конструкторов
		...
		bool correct1p1 = (obj1.GetIndict() == 1) && (obj1.GetSun() == 1) && (obj1.GetMoon() == 1);
	...
	{//Test5. Проверка работоспособности перегрузки операции помещения в поток и взятия из потока в различных формах
		...
		bool correct5p1 = output == "Indict: 15, CircleToSun: 13, CircleToMoon: 10| YearFromAdam: 1245| YearFromJesus: -4263| Indict: 15, CircleToSun: 13, CircleToMoon: 10";
