...
BigNumber BigNumber::operator+ (const BigNumber& right) const
{
	...
	if (this->sign == 0 && right.sign == 0)
	{
		...
		{
			int digit1 = (first.number[i] - '0');
			int digit2 = (second.number[i] - '0');
			int cnt = i-1;
			...
			if (digit1 + digit2 >= 10)
			{
				result.number[i+1] = (digit1 + digit2) % 10 + '0';
				if ((first.number[i-1]-'0') + 1 == 10)
				{
				...
				else { first.number[i-1] += 1; }
			}
			else { result.number[i+1] = digit1 + digit2 + '0'; }
...
BigNumber  BigNumber::operator* (const BigNumber& right) const
{
	...
	// из-за переполнения char'a при данном алгоритме перейдем от string к vector<long>

	std::vector<long> firstVec, secondVec, resultVec(this->length() + right.length() + 1);
